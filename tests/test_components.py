from graph.services.component_service import ComponentService


def test_empty_graph_components():
    # TODO: test graph with no users
    pass


def test_single_user_component():
    # TODO: test graph with one isolated user
    pass


def test_one_connected_component():
    # TODO: test graph where all users are connected
    pass


def test_multiple_connected_components():
    # TODO: test graph with multiple separate groups
    pass


def test_largest_component():
    # TODO: test largest component detection
    pass


def test_equal_size_largest_components():
    # TODO: test when multiple components have same max size
    pass


def test_component_with_cycle():
    # TODO: test DFS/BFS does not loop forever in cycle
    pass